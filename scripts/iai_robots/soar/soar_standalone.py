...
class WorldWithSOARConfig(WorldWithDiffDriveRobot):
    def __init__(self, urdf, map_name='map', localization_joint_name='localization', odom_link_name='odom', drive_joint_name='brumbrum'):
        super().__init__(urdf=urdf, map_name=map_name, localization_joint_name=localization_joint_name, odom_link_name=odom_link_name, drive_joint_name=drive_joint_name)

class SOARStandaloneInterface(StandAloneRobotInterfaceConfig):
    def __init__(self, drive_joint_name = 'brumbrum'):
        super().__init__([drive_joint_name, 'wheel_left_joint', 'wheel_right_joint', 'torso_joint', 'head_pan_joint', 'head_tilt_joint','arm_joint1', 'arm_joint2', 'arm_joint3', 'arm_joint4', 'arm_joint5', 'arm_joint6', 'arm_joint7', 'arm_drive_joint'])
...
class SOARCollisionAvoidanceConfig(CollisionAvoidanceConfig):
    ...
    def setup(self):
        ...
        self.overwrite_external_collision_avoidance(self.drive_joint_name, number_of_repeller=2, soft_threshold=0.2, hard_threshold=0.1)
